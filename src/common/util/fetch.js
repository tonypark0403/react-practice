...
const apiCache = new lruCache({
  max: 500, // the number of caching data
  maxAge: 1000 * 60 * 2, // remember for only 2m
});
...
export function makeFetchSaga({
  fetchSaga,
  canCache,
  getTotalCount = (res) => res?.totalCount, // for pagination
}) {
  return function* (action) {
    ...
    const iterStack = [];
    let iter = fetchSaga(action, page); // saga function we write
    let res;
    let checkSlowTask;
    let params;
    while (true) {
      const { value, done } = iter.next(res); // saga function returns the values
      if (getIsCallEffect(value) && getIsGeneratorFunction(value.payload.fn)) {
        // in case of calling saga function in saga function
        iterStack.push(iter);
      ...
      if (getIsCallEffect(value) && value.payload.fn === callApi) {
        ...
        if (!isFromCache) {
          if (!apiResult) {
            ...
            if (checkSlowTask) {
              yield cancel(checkSlowTask); // if the period of api call is less than .5s, cancel the status of isSlow
            }
          }
        }
        res = apiResult;
...
// need to sort key to recognize the same key regardless of query parameter's order
